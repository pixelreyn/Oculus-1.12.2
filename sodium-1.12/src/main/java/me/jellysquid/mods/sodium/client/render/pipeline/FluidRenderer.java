...
public class FluidRenderer {
    ...
    // Current fluid entity ID for shader packs (set per render call)
    private short currentEntityId;
    ...
    public boolean render(IBlockAccess world, IBlockState fluidState, BlockPos pos, ChunkModelBuffers buffers) {
        // Look up entity ID for this fluid (for shader pack mc_Entity attribute)
        this.currentEntityId = BlockRenderer.getEntityId(fluidState);
    ...
    private void flushQuad(ChunkModelBuffers buffers, ModelQuadView quad, ModelQuadFacing facing, boolean flip) {
        ...
        // Compute mid-texture coordinates (average of 4 vertices)
        ...
        // Compute tangent from facing direction
        int tangent = BlockRenderer.computeTangent(facing == ModelQuadFacing.UP ? EnumFacing.UP :
                facing == ModelQuadFacing.DOWN ? EnumFacing.DOWN :
                facing == ModelQuadFacing.NORTH ? EnumFacing.NORTH :
                facing == ModelQuadFacing.SOUTH ? EnumFacing.SOUTH :
                facing == ModelQuadFacing.EAST ? EnumFacing.EAST :
                facing == ModelQuadFacing.WEST ? EnumFacing.WEST : EnumFacing.UP);
        ...
        for (int i = 0; i < 4; i++) {
            ...
            sink.writeQuadExtended(x, y, z, color, u, v, light,
                    this.currentEntityId, midU, midV, tangent);
