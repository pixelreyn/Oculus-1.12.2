...
public class ShaderPack {
    ...
    public ShaderPack(Path root, Map<String, String> changedConfigs, Iterable<StringPair> environmentDefines) throws IOException, IllegalStateException {
        ...
        if (!invalidFeatureFlags.isEmpty()) {
            // In 1.12.2, we don't have the GUI screens, so just log the error and disable shaders
            Iris.logger.error("Shader pack requires unsupported features: " +
                    invalidFeatureFlags.stream().collect(Collectors.joining(", ")));
            Iris.logger.error("Status: " + FeatureFlags.getInvalidStatus(invalidFlagList));
