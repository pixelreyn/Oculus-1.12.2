...
public class PBRTextureManager {
    ...
    private static void closeTexture(AbstractTexture texture) {
        // In 1.12.2, AbstractTexture doesn't have close() method
    ...
    private PBRTextureHolder loadHolder(int id) {
        ...
        if (texture != null) {
            ...
            if (loader != null) {
                int previousTextureBinding = org.lwjgl.opengl.GL11.glGetInteger(org.lwjgl.opengl.GL11.GL_TEXTURE_BINDING_2D);
                consumer.clear();
                try {
    ...
    public void close() {
        clear();
        // In 1.12.2, use deleteTexture instead of close()
        GlStateManager.deleteTexture(defaultNormalTexture.getGlTextureId());
        GlStateManager.deleteTexture(defaultSpecularTexture.getGlTextureId());
    }
