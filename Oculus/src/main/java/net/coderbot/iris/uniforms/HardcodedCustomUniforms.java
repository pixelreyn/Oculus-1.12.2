...
public class HardcodedCustomUniforms {
    ...
    public static void addHardcodedCustomUniforms(UniformHolder holder, FrameUpdateNotifier updateNotifier) {
        updateNotifier.addListener(() -> {
            if (Minecraft.getMinecraft().world != null && Minecraft.getMinecraft().getRenderViewEntity() != null) {
                BlockPos pos = Minecraft.getMinecraft().getRenderViewEntity().getPosition();
                storedBiome = Minecraft.getMinecraft().world.getBiome(pos);
            } else {
                storedBiome = null;
            }
        });
        ...
        // BSL biome category uniforms
        holder.uniform1f(UniformUpdateFrequency.PER_FRAME, "isJungle", new SmoothedFloat(5, 5, () -> biomeNameContains("jungle") ? 1 : 0, updateNotifier));
        holder.uniform1f(UniformUpdateFrequency.PER_FRAME, "isMushroom", new SmoothedFloat(5, 5, () -> biomeNameContains("mushroom") ? 1 : 0, updateNotifier));
        holder.uniform1f(UniformUpdateFrequency.PER_FRAME, "isSavanna", new SmoothedFloat(5, 5, () -> biomeNameContains("savanna") ? 1 : 0, updateNotifier));
        holder.uniform1f(UniformUpdateFrequency.PER_FRAME, "isSwamp", new SmoothedFloat(5, 5, () -> biomeNameContains("swamp") ? 1 : 0, updateNotifier));
        holder.uniform1f(UniformUpdateFrequency.PER_FRAME, "isCold", new SmoothedFloat(5, 5, () -> {
            if (storedBiome == null) return 0f;
            return storedBiome.getDefaultTemperature() < 0.2f ? 1f : 0f;
        }, updateNotifier));
        holder.uniform1f(UniformUpdateFrequency.PER_FRAME, "isMesa", new SmoothedFloat(5, 5, () -> biomeNameContains("mesa") || biomeNameContains("badlands") ? 1 : 0, updateNotifier));
        holder.uniform1f(UniformUpdateFrequency.PER_FRAME, "isDesert", new SmoothedFloat(5, 5, () -> biomeNameContains("desert") ? 1 : 0, updateNotifier));
        ...
        holder.uniform1f(UniformUpdateFrequency.PER_FRAME, "inSwamp", new SmoothedFloat(5, 5, () -> {
            if (storedBiome == null) {
                ...
                // Check for swamp biome in 1.12.2
                return storedBiome.getBiomeName().toLowerCase().contains("swamp") ? 1 : 0;
            }
        }, updateNotifier));
        holder.uniform1f(UniformUpdateFrequency.PER_FRAME, "BiomeTemp", () -> {
            if (storedBiome == null || client.getRenderViewEntity() == null) {
                return 0;
            } else {
                return storedBiome.getTemperature(client.getRenderViewEntity().getPosition());
            }
        });
    ...
    private static float getBurnFactor() {
        return Minecraft.getMinecraft().player != null && Minecraft.getMinecraft().player.isBurning() ? 1.0f : 0f;
    }

    private static float getSneakFactor() {
        return Minecraft.getMinecraft().player != null && Minecraft.getMinecraft().player.isSneaking() ? 1.0f : 0f;
    }
    ...
    private static float getEyeInCave() {
        if (client.getRenderViewEntity() == null) return 0.0f;
        if (client.getRenderViewEntity().posY + client.getRenderViewEntity().getEyeHeight() < 5.0) {
    ...
    private static float getEyeSkyBrightness() {
        ...
        BlockPos eyeBlockPos = new BlockPos(
            client.getRenderViewEntity().posX,
            client.getRenderViewEntity().posY + client.getRenderViewEntity().getEyeHeight(),
            client.getRenderViewEntity().posZ
        );
    ...
    private static int getWorldDayTime() {
        ...
        long timeOfDay = world.getWorldTime();
        // In 1.12.2, just use mod directly - no fixed time OptionalLong
    ...
    private static float getRawPrecipitation() {
        ...
        // 1.12.2 biome precipitation check
        if (!storedBiome.canRain() && !storedBiome.isSnowyBiome()) {
            return 0; // Dry
        } else if (storedBiome.isSnowyBiome()) {
            return 2; // Snow
        } else {
            return 1; // Rain
        }
    ...
    private static boolean biomeNameContains(String name) {
        if (storedBiome == null) return false;
        return storedBiome.getBiomeName().toLowerCase().contains(name);
    }
